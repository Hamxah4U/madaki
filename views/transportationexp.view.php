...
<div id="wrapper">
  ...
  <div id="content-wrapper" class="d-flex flex-column">
              ...
              <table class="table table-striped text-nowrap" id="peopleTable">
                ...
                <tbody id="tableBody">
                  <tr>
                    ...
                    <td><input type="number" name="total_animal[]" style="width: 66px;"
                        value="<?= $editData['total_animal'] ?? '' ?>" class="form-control" <?= $ro ?>></td>
                    <td><input type="number" name="death_animal[]" style="width: 66px;"
                        value="<?= $editData['death_animal'] ?? '' ?>" class="form-control"></td>
                    <td><input type="number" name="surviving_animal[]" style="width: 66px;"
                        value="<?= $editData['surviving_animal'] ?? '' ?>" class="form-control" <?= $ro ?>></td>
                    <!-- <td><input type="number" name="amount[]" value="<?php // $editData['amount'] ?? '' 
                                                                                                ?>" class="form-control"></td> -->
                    <td><input type="number" name="first_payment[]" style="width: 85px;"
                        value="<?= $editData['first_payment'] ?? '' ?>" class="form-control"></td>
                    <td><input type="number" name="second_payment[]" style="width: 85px;"
                        value="<?= $editData['second_payment'] ?? '' ?>" class="form-control"></td>
                    <td><input type="number" name="third_payment[]" style="width: 85px;"
                        value="<?= $editData['third_payment'] ?? '' ?>" class="form-control"></td>
                    <td><input type="number" name="total[]" style="width: 86px;" value="<?= $editData['total'] ?? '' ?>"
                        class="form-control" <?= $ro ?>></td>
            ...
            <table class="table table-bordered text-nowrap">
              <div id="printGroupMarket">
                ...
                <tbody>
                  ...
                  <tr class="<?= $rowClass ?>" data-market-group="<?= htmlspecialchars($row['name']) ?>">
                    <td><?= $i + 1 ?></td>
                    <td><?= $row['fullname'] ?></td>
                    ...
                    <td class="no-print">
                      ...
                      <button class="btn btn-sm btn-info receiptBtn" data-id="<?= $row['id'] ?>"
                        data-phone="<?= $row['phone'] ?>" data-tid="<?= $transport_id ?>" data-bs-toggle="modal"
                        data-bs-target="#receiptModal">
                        Receipt
                      </button>
                      <?php if($row['status_id'] == 1 && $_SESSION['role'] == 'Agent') : ?>
                      <a href="?id=<?= $transport_id ?>&edit=<?= $row['id'] ?>" class="btn btn-sm btn-primary">Receive
                        Money</a>
                      ...
                      <?php if ($_SESSION['role'] == 'Admin'): ?>
                      <a href="/delete-exp?id=<?= $row['id'] ?>&tid=<?= $transport_id ?>" class="btn btn-sm btn-danger"
                      ...
                      <tr>
                        ...
                        <td class="no-print">
                          <a href="/delete-comment?id=<?= $row_comment['id'] ?>&tid=<?= $transport_id ?>"
                            class="btn btn-sm btn-danger" onclick="return confirm('Delete this record?')">
                            Delete
                          </a>
                          <button class="btn btn-info btn-edit-comment" data-id="<?= $row_comment['id'] ?>"
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="no-print">
                          ...
                          <button class="btn btn-info btn-edit-othercomment" data-id="<?= $row_comment['id'] ?>"
                            data-comment="<?= htmlspecialchars($row_comment['reason']) ?>">
                            Edit
                          </button>
                          <?php endif ?>
                  ...
                  <table class="table table-bordered text-nowrap">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="no-print">
                          <a href="/delete-other-expenses?id=<?= $row_diary['id'] ?>&tid=<?= $transport_id ?>"
                            class="btn btn-sm btn-danger no-print"
                            onclick="return confirm('Delete this record?')">Delete</a>
                          <!-- <a href="/edit">Edit</a> -->
                          <button class="btn btn-info btn-edit-diary" data-id="<?= $row_diary['id'] ?>"
    ...
    <script>
        ...
        // 1. Get the group name from the button data attribute
        let groupName = $(this).data('group');

        // 2. Get the Driver Info table html (Header)
        let headHtml = $('#headArea').html();

        // 3. Clone matching rows AND find the Subtotal row right after them
        let rowsHtml = '';
        $('tr[data-market-group="' + groupName + '"]').each(function() {
            let clonedRow = $(this).clone();
            clonedRow.find('.no-print').remove(); // strip action columns
            rowsHtml += clonedRow.prop('outerHTML');
        });

        // --- NEW: Find and append the Subtotal row for this specific market ---
        // Look for the last matching record row, then grab the next neighboring subtotal row element
        let lastRecordRow = $('tr[data-market-group="' + groupName + '"]').last();
        let subtotalRow = lastRecordRow.nextAll('tr').filter(function() {
            return $(this).text().indexOf('Subtotal (' + groupName + ')') !== -1;
        }).first();

        if (subtotalRow.length > 0) {
            rowsHtml += subtotalRow.clone().prop('outerHTML');
        }

        // 4. Construct the standalone printing layout structure
        let printWindowContent = `
            <html>
            <head>
                <title>Print - Market: ${groupName}</title>
                <link href="css/sb-admin-2.min.css" rel="stylesheet">
                <style>
                    body { padding: 20px; font-family: sans-serif; background-color: #fff !important; color: #000; }
            ...
            <body>
                ...
                <h4 style="margin-top:25px; border-bottom: 2px solid #000; padding-bottom: 5px;">
                    Market: ${groupName}
                </h4>
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ${rowsHtml ? rowsHtml : '<tr><td colspan="11" class="text-center">No records found for this market group.</td></tr>'}
                    </tbody>
                </table>
            </body>
            </html>
        ...
        // 5. Open new context window, print, and safely close it
        let win = window.open('', '_blank', 'width=900,height=700');
        win.document.write(printWindowContent);
        win.document.close();
